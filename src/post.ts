...
import { postToSlack } from './providers/slack';
import { postViaZapier } from './providers/zapier';
...
// --video-name=NAME (Google Sheets の video_name 列向け)
const vnameArg = process.argv.find((a) => a.startsWith('--video-name='));
const videoName = vnameArg ? vnameArg.replace('--video-name=', '') : undefined;
...
async function main() {
  ...
  await postViaZapier(target as any, finalMsg, {
    image: zapierImage,
