...
work   = root/"media/work"; work.mkdir(parents=True, exist_ok=True)
outdir = root/"media/video"; outdir.mkdir(parents=True, exist_ok=True)
...
if not voice.exists():
    # ない場合はダミー無音3秒
    subprocess.run(["ffmpeg","-f","lavfi","-t","3","-i","anullsrc=r=44100:cl=stereo","-y",str(voice)], check=True)
...
cmd = [
  ...
  "-i", str(voice),
  "-t","15",
  "-vf", f"drawtext=textfile='{txt}':fontcolor=white:fontsize=36:box=1:boxcolor=0x00000088:boxborderw=12:x=(w-text_w)/2:y=h-200",
