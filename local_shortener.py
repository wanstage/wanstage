...
def load_links():  return _load(DB_LINKS, {})
def save_links(d): 
    with _lock: _save(DB_LINKS, d)

def load_clicks(): return _load(DB_CLICKS, {})
def save_clicks(d):
    with _lock: _save(DB_CLICKS, d)
...
def stats(code: str, request: Request):
    ...
    links = load_links()
    clicks = load_clicks()
    return {"code": code, "long_url": links.get(code), "stats": clicks.get(code, {"total":0,"byDay":{}})}
...
def head_redirect(code: str):
    if code not in load_links(): raise HTTPException(404)
    return {}
...
def go(code: str, request: Request):
    ...
    if not long_url: raise HTTPException(404)
